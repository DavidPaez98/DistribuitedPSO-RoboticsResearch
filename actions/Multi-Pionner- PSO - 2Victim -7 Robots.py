...
C = [0.1,0.2,0.3,0.45,0.65] # Coeficientes PSO
dt = 0.5 # Paso o Step en la actualizacion PSO

Dist_Enjambre = 2 # Distancia Minima - Desviacion estandar
Dist_Particula = 0.5 # Distancia Minima - Desviacion estandar   
...
Alt_Victim = -4 #-2  # Altura de la Gaussiana en la construccion de la Superficie - Victimas
Alt_Obsta = 4    # Altura de la Gaussiana en la construccion de la Superficie - Obstaculos

Std_Victim = 1   # Grosor de la Gaussiana en la construccion de la Superficie - Victimas
Std_Obsta = 10     # Grosor de la Gaussiana en la construccion de la Superficie - Obstaculos
...
x_max=y_max = 25 
epocas = 3000 # Numero de Iteraciones
...
for Robot in range(Number_Robots):
    ...
    errorCode, PosRobot  = vrep.simxGetObjectPosition(clientID,pioneer_handle_,-1,vrep.simx_opmode_blocking)    
...
for Victim in range(Number_Victims):
    _victim_name = victim_name + '#' + str(Victim)
    errorCode, Victim_handle =vrep.simxGetObjectHandle(clientID,_victim_name,vrep.simx_opmode_oneshot_wait)
    errorCode, PosVictim = vrep.simxGetObjectPosition(clientID,Victim_handle,-1,vrep.simx_opmode_blocking)
...
for Obstacle in range(Number_Obstacles):
    _obstacle_name = obstacle_name + '#' + str(Obstacle)
    errorCode, Fire_handle =vrep.simxGetObjectHandle(clientID,_obstacle_name,vrep.simx_opmode_oneshot_wait)
    errorCode, PosFire = vrep.simxGetObjectPosition(clientID,Fire_handle,-1,vrep.simx_opmode_blocking)
...
ax.scatter(HistPosPar[0,1,:,0],HistPosPar[0,1,:,1], s=50, c='r', marker='o',zorder=10, label="Particles Init Position")
ax.plot(HistPosTarget[0,:,0],HistPosTarget[0,:,1],zorder=5, c='k', linewidth=1.5, label="Mean Position")
ax.scatter(HistPosPar[0,epocas-1,0,0],HistPosPar[0,epocas-1,0,1], s=50, c='k', marker='o', zorder=10, label="Particles Final Position")
...
for Robot in range(Number_Robots):
    ...
    ax.plot(HistPosTarget[Robot,:,0],HistPosTarget[Robot,:,1],zorder=25,c='k', linewidth=1.5)
    ...
    for particula in range(Part_Enjambre):
        ax.plot(HistPosPar[Robot,:,particula,0],HistPosPar[Robot,:,particula,1],c='#2737AE')
    
    for Pos_Med in range(epocas):
        #print(Pos_Med)
        if Pos_Med % 200 == 0:            
            ax.scatter(HistPosTarget[Robot,Pos_Med,0],HistPosTarget[Robot,Pos_Med,1], s=10, c='w', marker='o', zorder=10)
        
ax.scatter(HistPosTarget[:,epocas-1,0],HistPosTarget[:,epocas-1,1], s=10, c='b', marker='o',zorder=20, label="Mean Final Position")       
ax.legend()       
ax.grid()
...
ax.scatter(HistPosTarget[:,epocas-1,0],HistPosTarget[:,epocas-1,1], s=50, c='r', marker='o')
ax.grid()
