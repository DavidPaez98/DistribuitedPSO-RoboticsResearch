...
class Particula(object):
    ...
    def actualizarVel(self, bestPosSwr, C):
        ...
        for i in range(len(bestPosSwr)):
             self.velocidad[i]= C[0]*self.velocidad[i]  +  C[1]*((np.random.random()/2)+0.5)*(Norm_PosPart[i])  + C[2]*((np.random.random()/2)+0.5)*(Norm_PosSwrm[i])  + C[3]*((np.random.random()/2)+0.5)*(self.repulsionPart[i]) + C[4]*((np.random.random()/2)+0.5)*(self.repulsionSwr[i])
...
def funEval3D(posicion):
    ...
    Z_Global_ = makeGaussian(xRobot,yRobot,-10,0,45,200,40,0.5)
...
Number_Robots = 26 #Numero de Robots
Number_Victims = 3 #Numero de Victima        Bill#0 y Bill#1
Number_Obstacles = 10 #Numero de Obstaculos  Fire#0 a Fire#11
Part_Enjambre = 20# Particulas por Enjambre

C = [0.1,0.5,1.5,1,0.5] # Coeficientes PSO   #  0=velocdad  1=particula   2=swarm   3=repulsion part   4= repulsion swarm
dt = 0.2#0.2 # Paso o Step en la actualizacion PSO

Dist_Enjambre = 3 # 3  Distancia Minima - Desviacion estandar
...
Alt_Victim = -2 #-2,  -8  # Altura de la Gaussiana en la construccion de la Superficie - Victimas
Alt_Obsta = 9 #4   # Altura de la Gaussiana en la construccion de la Superficie - Obstaculos

Std_Victim = 3   # 10 Grosor de la Gaussiana en la construccion de la Superficie - Victimas
...
x_max=y_max = 50 
epocas = 1800 # Numero de Iteraciones
